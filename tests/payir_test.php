...
class enrol_payir_testcase extends advanced_testcase {
    ...
    public function test_basics() {
        $this->assertFalse(enrol_is_enabled('payir'));
        $plugin = enrol_get_plugin('payir');
        $this->assertInstanceOf('enrol_payir_plugin', $plugin);
        $this->assertEquals(ENROL_EXT_REMOVED_SUSPENDNOROLES, get_config('enrol_payir', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $this->enable_plugin();
        $payirplugin = enrol_get_plugin('payir');

        // Just make sure the sync does not throw any errors when nothing to do.
        $payirplugin->sync(new null_progress_trace());
    }
    ...
    public function test_expired() {
        ...
        /** @var enrol_payir_plugin $payirplugin  */
        $payirplugin = enrol_get_plugin('payir');
        ...
        $payirplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $payirplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $payirplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $payirplugin->enrol_user($instance2, $user1->id, $studentrole->id, 0, 0);
        $payirplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now-60*60);
        $payirplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now+60*60);

        $payirplugin->enrol_user($instance3, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $payirplugin->enrol_user($instance3, $user4->id, $teacherrole->id);
        ...
        $payirplugin->set_config('expiredaction', ENROL_EXT_REMOVED_KEEP);
        $code = $payirplugin->sync($trace);
        $this->assertSame(0, $code);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(9, $DB->count_records('role_assignments'));


        $payirplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $payirplugin->sync($trace);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        ...
        $payirplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
    ...
    public function test_get_user_enrolment_actions() {
        ...
        // Only enable the payir enrol plugin.
        ...
        // Payir enrolment has 2 enrol actions for active users when logged in as admin: edit and unenrol.
        ...
        $actions = $plugin->get_user_enrolment_actions($manager, $ue);
        // Teachers don't have the enrol/payir:unenrol capability by default, but have enrol/payir:manage.
